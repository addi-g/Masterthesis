...
\begin{defn}
\label{ueberdeckung}
$(X, \delta)$ sei ein pseudometrischer Raum (siehe z.\@B.\@ \cite[Definition 2.1.1]{Topologie2015}). Für $x \in X$ und $\epsilon > 0$ sei:
...
Mit Ungleichung~(\ref{anc}) erhalten wir
\begin{equation}
\E \bigg[\int |m_n(x) - m(x)|^2 \mathds{P}_X(dx)\bigg] \leq \frac{4 \cdot c_7 \cdot \beta_n^2}{n} + T_{2,n}
\end{equation}
und wollen nun den zweiten Summanden abschätzen.
...
auf Ereignis $A_n$, woraus durch $f_j \in \mathcal{F}$ schließlich $\tilde{m}_n \in \mathcal{F}^{(J_n)}$ folgt.
...
Wir wollen beide Summanden aus Ungleichung~(\ref{eq:lem8}) weiter abschätzen.
...
\end{split}
\end{equation*}
wie in Gleichung~(\ref{konvexkomb}) eine lokale Spline-Interpolation von Taylorpolynomen von $m$ ist.
...
& \eqqcolon 2 \sup_{x \in [-a_n, a_n]^d} T_{2,B_1}^2 + 2 \sup_{x \in [-a_n, a_n]^d} T_{2,B_2}^2,
\end{split}
\end{equation}
...
\\
Dafür überprüfen wir, ob dafür alle Voraussetzungen erfüllt sind. Wir betrachten wieder den logistischen Squasher $\sigma$ aus Gleichung~\ref{logsquasher}, welcher nach Lemma~\ref{lem:logsquasher} insbesondere 2-zulässig ist. Zudem ist für hinreichend großes $n$ die Bedingung 
\begin{equation*}
\begin{split}
