...
\section{Die Abschätzung des erwarteten $L_2$-Fehlers}
...
im Falle des Schätzers $$m_n(x) \coloneqq T_{\beta_n}\tilde{m}_n (x),$$ unter Annahme einer $(p,C)$-glatten Regressionsfunktion $m$ herzuleiten. Wobei $T_{\beta}z = \max\{\min\{z, \beta\}, -\beta\}$ für $z \in \R$ und $\beta > 0$, und $\tilde{m}_n$ unser Neuronale-Netze-Regresssionsschätzer aus Kapitel~\ref{subsec:2.2} ist, welcher aus mehreren neuronalen Netzen konstruiert wurde.

\begin{mthm}\label{optstop}
...
Wir betrachten den Neuronale-Netze-Regressionsschätzer $\tilde{m}_n$, mit Aktivierungsfunktion $\sigma$ als den logistischen Squasher. Aufgrund der Konstruktion unseres Schätzers wählen wir zudem $N \geq q, M = M_n = \lceil c_2 \cdot n^{1/(2p + d)}\rceil$ mit $c_2 >0$ und unabhängig von $n$, $R = R_n = n^{d + 4}$ und $a = a_n = (\log n)^{1/(6(N + d))}.$
...
wobei $c_{\mathrm{fin}} > 0$ ist und nicht von $n$ abhängt.
\end{mthm}
...
und 
$$\frac{1}{n} \sum_{i = 1}^n |Y_i - \tilde{m}_n(X_i)|^2 \leq \min_{l \in \Theta_n}\bigg(\frac{1}{n}\sum_{i = 1}^n |Y_i - g_{n,l}(X_i)|^2 + \mathrm{pen}_n(g_n,l)\bigg)$$
mit einer nichtleeren Parametermenge $\Theta_n$, zufällige Funktionen $g_{n,l}\colon \R^d \to \R$ und deterministischen Penalty Terme $\mathrm{pen}_n(g_{n,l}) \geq 0$, wobei die zufälligen Funktionen $g_{n,l}\colon \R^d \to \R$ nur von den Zufallsvariablen
...
& \quad + 2 \cdot \E\bigg(\min_{l \in \Theta_n} \int |g_{n,l}(x) - m(x)|^2 \mathds{P}_X(dx) + \mathrm{pen}_n(g_{n,l})\bigg),
\end{split}
\end{equation*}
...
\begin{proof}[Beweis von Hauptsatz~\ref{optstop}]
