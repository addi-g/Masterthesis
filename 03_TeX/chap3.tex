...
Mit Ungleichungen~(\ref{anc}) und~(\ref{6thsum}) erhalten wir
\begin{equation}
...
\end{split}
\end{equation}
Nun haben wir alle Summanden von Ungleichung ($\ref{originaleq}$) abgeschätzt und erhalten schließlich mit Ungleichungen~(\ref{6thsum})-(\ref{4thsum}):
\begin{equation*}
\E \Big[\int |m_n(x) - m(x)|^2 \mathds{P}_X(dx)\Big] \leq c_{\mathrm{fin}} \cdot \log(n)^3 \cdot n^{- \frac{2p}{2p + d}},
\end{equation*}
mit 
