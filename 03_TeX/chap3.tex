...
Sei $\sigma$ 2-zulässig nach Definition~\ref{nzulässig}. Sei $\mathcal{F}$ die Menge aller Funktionen die durch Definition~\ref{def:nn} definiert sind mit $k_1 = k_2 = \cdots = k_L = 24 \cdot (N + d)$ und dass der Betrag der Gewichte durch $c_{3} \cdot n^{c_{4}}$ beschränkt ist. Sei
...
\end{split}
\end{equation}
Wir haben bei Ungleichung~(\ref{anc}) zudem verwendet, dass wir $n$ und $c_3$ so groß wählen, dass $\max\{\|m\|_{\infty}, \|m_n\|_{\infty}\} < \beta_n$ gilt. Bei der letzten Gleichung habe wir schließlich verwendet dass $\beta_n$ deterministisch und $\mathds{P}(X \in \supp(\mathds{P}_X)) = 1$ ist.
...
\end{split}
\end{equation} 
mit von $n$ unabhängigen Konstanten $c_{13} \coloneqq c_8 \cdot 2^d > 0$ und $c_{14} \coloneqq 4 \cdot c_4 \cdot c_3^2 > 0$.
