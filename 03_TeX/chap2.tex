...
Die Netzwerkarchitektur $(L, k)$ beschreibt die Gestalt eines neuronalen Netzes. Durch $L \in \N_0$ wird die Anzahl der verborgenen Schichten angeben und der Vektor $k = (k_1,\dots,k_L) \in \N^L$ gibt mit jeder Komponente die Anzahl der Neuronen an, die in der jeweiligen verborgen Schicht enthalten sind. 

Ein mehrschichtiges feedforward neuronales Netz mit Architektur $(L, k)$ und $\sigma$ aus (\ref{logsquasher}) als Aktivierungsfunktion, ist eine reellwertige Funktion $f\colon \R^d \to \R$ definiert durch:
\begin{align}
\label{networkarch}
...
%Bei neuronale Netze Regressionsschätzer wird für die Ausgabeschicht keine Aktivierungsfunktion gewählt, da wir Funktionswerte schätzen möchten und es sich hier nicht um ein Klassifizierungssproblem handelt.
...
\begin{equation*}
\begin{split}
|f(x) - P(x)| & \leq \sum_{\bi \in [M]^d} \bigg|f(x) - \sum_{\substack{\mathbf{j} \in [q]^d \\|\mathbf{j}|_1 \leq q}} \partial^{\mathbf{j}}f(x_{\mathbf{i}}) \cdot \frac{(x - x_{\mathbf{i}})^{\mathbf{j}}}{\mathbf{j}!}\bigg| \prod_{j = 1}^d \bigg(1 - \frac{M}{2a} \cdot |x^{(j)} - x_{\mathbf{i}}^{(j)}|\bigg)_+ \\
