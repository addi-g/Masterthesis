...
$$
das \emph{Taylorpolynom der Ordnung $N$} von $f$ in $a$ ist und das Restglied mit einem geeigneten Punkt $\xi \in [u,x]$ in der Form 
...
Wir betrachten erst eine Approximation der Identität durch das neuronale Netz $f_{\id}$ mit Architektur $(1,(1))$ welches in Abbildung~\ref{fig:fid} veranschaulicht wird und anschließend die Approximation eines Quadrats auf einem Intervall durch das neuronale Netz $f_{\sq}$.
\begin{figure}[htp]
\centering
\begin{tikzpicture}[
plain/.style={
...
\end{proof}
Wir betrachten als nächstes die Approximation einer Multiplikation zweier Werte auf einem Intervall durch das neuronale Netz $f_{\mult}$ mit Architektur $(1,(4))$ welches in Abbildung~\ref{fig:fmult} veranschaulicht wird.
\begin{figure}[htp]
\centering
\begin{tikzpicture}[
plain/.style={
...
wobei wir bei der letzten Ungleichung Lemma~\ref{lem:1}b) mit $x + y, x - y \in [-2a, 2a]$ verwendet haben. Daher folgt insbesondere für beliebiges $x \in [-2a,2a]$ $$ |f_{\sq}(x) - x^2| \leq \frac{40 \cdot \|\sigma'''\|_{\infty} \cdot a^3}{3 \cdot |\sigma''(t_{\sigma})|} \cdot \frac{1}{R}$$ gilt. 
  \end{proof}
Wir betrachten als nächstes die Approximation der Maximumsfunktion $\max\{x, 0\}$ für $x$ aus einem Intervall, durch das neuronale Netz $f_{\ReLU}$ mit Architektur $(2,(2,4))$ Abbildung~\ref{fig:frelu} veranschaulicht wird.
\begin{figure}[htp]
\centering
\begin{tikzpicture}[
plain/.style={
...
Daraus folgt die Behauptung.
  \end{proof}
Wir betrachten als nächstes die Approximation des Positivteils einer Funktion auf einer Intervall durch das neuronale Netz $f_{\mathrm{hat},y}$ mit Architektur $(2, (6, 12))$ welches Abbildung~\ref{fig:fhat} veranschaulicht wird.
\begin{figure}[htp]
\centering
\begin{tikzpicture}[
plain/.style={
