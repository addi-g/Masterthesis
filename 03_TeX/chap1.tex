...
\end{defn}
\begin{bemnumber}
Eine Funktion ist genau dann \emph{$s$-hölderstetig} mit \emph{Hölderkonstante $C$}, wenn. $p = s = 1$ gilt.
\end{bemnumber}
...
Nun zu Bedingung (iii). Wir wissen, dass für $x \in \R$ und damit insbesondere für ein beliebiges $x > 0$ die Ungleichung $$ x \leq \exp(x) + 1$$ gilt. Daraus erhalten wir mit Umformungen da $x > 0$ und $1 + \exp(-x) > 0$ ist:
\begin{equation*}
\begin{split}
& \quad x \leq \exp(x) + 1 \\
