...
\item[(ii)] Es existiert ein Punkt $t_{\sigma} \in \R$ mit $$\frac{\partial^i \sigma}{\partial x^i}(t_{\sigma}) \neq 0 \text{ für } i = 0,\dots,N.$$
...
\begin{equation}
\label{eq:leibniz}
\frac{\partial^n\sigma}{\partial x^n} = \frac{\partial^{n - 1}}{\partial x^{n - 1}}\bigg((1 - \sigma) \cdot \sigma\bigg) = \sum_{k = 0}^{n-1}\binom{n - 1}{k}(1 - \sigma)^{(n - 1 - k)}  \sigma^{(k)} 
\end{equation}
für $n = 1,\dots,N$. 
Damit erkennen wir durch auflösen der Rekursion, dass alle Ableitungen von $\sigma$, Polynome in $\sigma$ sind. Dadurch folgt Bedingung (i) aus Definition~\ref{nzulässig}, da $\sigma$ nach Voraussetzung beschränkt ist und die Ableitungen von $\sigma$ als endliche Summe von Produkten beschränkter Faktoren ebenfalls beschränkt ist. Da hiermit auch insbesondere die erste Ableitung von $\sigma$ beschränkt ist, wissen wir, dass $\sigma$ lipschitzstetig ist.

Nun kommen wir zum Beweis von Bedingung (ii). 
Wir wissen, dass Polynome die nicht das Nullpolynom sind nur endliche viele Nullstellen besitzen. Daher wissen wir durch Gleichung~(\ref{eq:leibniz}), dass ein $t_{\sigma} \in \R$ existiert, mit $$\frac{\partial^i \sigma}{\partial x^i}(t_{\sigma}) \neq 0 \text{ für } i = 0,\dots,N.$$
\todo{ausführlicher, warum ist das Nullpolynom ausgeschlossen? Surjektivität von $\sigma$ nutzen für die Existenz von $t_\sigma$}
...
Mit $f_{\id}(0) = 0$ und $f_{\id}'(0) = 1$ erhalten wir:
  	...
  	\begin{split}
  	 |f_{\id}(x) -  & x| \\
  	& = \bigg|0 + 1 \cdot x + \frac{1}{2} \cdot \frac{1}{R \cdot \sigma'(t_{\sigma,\id})} \sigma''\bigg(\frac{\xi}{R} + t_{\sigma,\id}\bigg) \cdot x^2 - x \bigg| \\
  	& = \bigg| \frac{\sigma''\left(\frac{\xi}{R} + t_{\sigma,\id}\right)  \cdot x^2}{2 \cdot R \cdot \sigma'(t_{\sigma, \id})} + x - x\bigg| \\
...
und 
$$
f_{\sq}''(x) = \frac{R^2}{\sigma''(t_{\sigma, \sq})} \cdot \left(\frac{4}{R^2} \cdot\sigma''\bigg(\frac{2x}{R} + t_{\sigma, \sq}\bigg) - \frac{2}{R^2} \cdot \sigma''\bigg(\frac{x}{R} + t_{\sigma, \sq}\bigg)\right).
$$
Mit $f_{\sq}(0) = 0$, $f_{\sq}'(0) = 0$ und $f_{\sq}''(0) = 2$ erhalten wir:
\begin{equation*}
  	\begin{split}
  	 & |f_{\sq}(x) - x^2| \\
  	& \; = \bigg|x^2 + \frac{1}{6} \cdot \frac{R^2}{\sigma''(t_{\sigma,\sq})} \left(\frac{8}{R^3}\sigma'''\bigg(\frac{2\xi}{R} + t_{\sigma,\sq}\bigg) - \frac{2}{R^3} \sigma'''\bigg(\frac{\xi}{R} + t_{\sigma,\sq}\bigg)\right) \cdot x^3 - x^2\bigg| \\
  	& \; \leq \frac{a^3}{6 \cdot |\sigma''(t_{\sigma,\sq})|} \cdot \frac{1}{R} \cdot \left(8 \cdot \bigg|\sigma'''\bigg(\frac{2\xi}{R} + t_{\sigma,\sq}\bigg)\bigg| + 2 \cdot \bigg|\sigma'''\bigg(\frac{\xi}{R} + t_{\sigma,\sq}\bigg)\bigg|\right) \\
...
\begin{lem}
  ...
  & \leq \frac{1}{4} \cdot \big|f_{\sq}(x + y) - (x + y)^2\big| + \frac{1}{4}\cdot\big|f_{\sq}(x - y) - (x - y)^2\big| \\
...
\begin{equation*}
\begin{split}
\bigg|f_{\mathrm{hat},y}(x) - & \bigg(1 - \frac{M}{2a} \cdot |x - y|\bigg)_+\bigg| \\
& \leq \bigg|f_{\ReLU} \bigg(\frac{M}{2a} \cdot (x - y) + 1\bigg) - \max\bigg\{\frac{M}{2a} \cdot (x - y) + 1, 0\bigg\}\bigg| \\ 
& \quad + 2 \cdot \bigg|f_{\ReLU}\bigg(\frac{M}{2a} \cdot (x - y)\bigg) - \max\bigg\{\frac{M}{2a} \cdot (x - y), 0\bigg\}\bigg| \\
& \quad + \bigg|f_{\ReLU}\bigg(\frac{M}{2a} \cdot (x - y) - 1\bigg) - \max\bigg\{\frac{M}{2a} \cdot (x - y) - 1, 0\bigg\}\bigg|\\ 
...
Um Gleichung~(\ref{lem:4:eq}) zu zeigen unterscheiden wir vier Fälle.
  \begin{itemize}
  \item[Fall 1] ($x < 0$) In diesem Fall hat die linke Seite von Gleichung~(\ref{lem:4:eq}) nach der Definition des Betrags die Gestalt $$\max\{1 + \frac{M}{2a} \cdot x, 0\}$$ und die rechte Seite von Gleichung~(\ref{lem:4:eq}) die Form $$\max\bigg\{\frac{M}{2a} \cdot x + 1, 0\bigg\} - 2 \cdot 0 + 0,$$ da $x < 0$ und damit die letzten zwei Summanden Null sind. Es erfordert hier eine weitere Fallunterscheidung.
