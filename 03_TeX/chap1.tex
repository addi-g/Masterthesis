...
In dieser Arbeit behandeln wir Neuronale-Netze-Regressionsschätzer im Kontext der nichtparametrischen Regression mit zufälligem Design. Im Gegensatz zur parametrischen Regression ist bei der nichtparametrischen die Bauart der zu schätzenden Funktion komplett unbekannt, was den Vorteil besitzt, dass weniger Annahmen getroffen werden müssen, man aber dadurch noch mehr Daten benötigt um eine Funktion zu schätzen.
...
Da wir uns in dieser Arbeit mit neuronalen Netzen zur Regressionschätzung beschäftigen und Funktionswerte schätzen wollen, betrachten wir nur neuronale Netze die in der Ausgabeschicht ein einzelnes Neuron besitzen. 
...
\begin{defn}
\label{nzulässig}
Sei $N \in \N_0$. Eine Funktion $\sigma\colon \R \to [0, 1]$ wird $N$-zulässig genannt, wenn sie monoton wachsend und lipschitzsteig (REFERENZ) ist und wenn zusätzlich die folgenden drei Bedingungen erfüllt sind$\colon$
\begin{itemize}
...
\end{proof}
\begin{lem}[Langrangesche Form des Restglieds \cite{forster}]
\label{lem:lagrange}
...
\begin{proof}
	\begin{itemize}
  	\item[a)] Wir wissen, dass $f_{\id}$ 2-mal differenzierbar ist, da nach Vorraussetzung $\sigma$ 2-mal stetig differenzierbar ist. Damit folgt mit Lemma \ref{lem:lagrange} für $f = f_{\id}, N = 1, u = 0$ und $I = [-a, a]$, dass ein $\xi \in [0, x]$ existiert, so dass:
$$f_{\id}(x) = \sum_{k = 0}^N \frac{f_{\id}^{(k)}(u)}{k!}(x - u)^k + \frac{f_{\id}^{(N + 1)}(\xi)}{(N + 1)!}(x - u)^{N + 1}.$$
Mit $f_{\id}(0) = 0$ und $f_{\id}'(0) = 1$ erhalten wir:
  	...
  	\item[b)] Folgt analog wie in a) durch Lemma \ref{lem:lagrange}auf die Funktion $f_{\\sq}$ die hier nun 3-mal differenzierbar ist, da $\sigma$ nach Voraussetzung  3-mal stetig differenzierbar ist.
 	\end{itemize}
\end{proof}
...
\begin{lem}
  ...
  \end{split}
\end{equation*}   
wobei bei bei der letzten Ungleichung verwendet haben, dass $a > 0$ nach Lemma \ref{lem:1}b) beliebig gewählt wurde und daher insbesondere für beliebiges $x \in [-2a,2a]$ $$ |f_{\sq}(x) - x^2| \leq \frac{40 \cdot \|\sigma'''\|_{\infty} \cdot a^3}{3 \cdot |\sigma''(t_{\sigma})|} \cdot \frac{1}{R}$$ gilt. 
  \end{proof}
  \begin{lem}
  \label{lem:3}
...
\begin{equation}
\label{lem:4:eq}
 (1 - \frac{M}{2a} \cdot |x|)_+ = \max\{\frac{M}{2a} \cdot x + 1, 0\} - 2 \cdot \max\{\frac{M}{2a} \cdot x, 0\} + \max\{\frac{M}{2a} \cdot x - 1, 0\}, \qquad (x \in \R)
 \end{equation}  
 damit wir das Resultat mithilfe von Lemma \ref{lem:3} beweisen können.
...
\end{itemize} 
Durch diese Fallunterscheidung wurde die Gleichung (\ref{lem:4:eq}) bewiesen. Daraus folgt mit der Definition von $f_{\mathrm{hat},y}(x)$ und zwei mal der Dreiecksungleichung
\begin{equation*}
\begin{split}
...
$$ (M + 1)^3 \leq (2M)^3 = 8M^3$$ verwendet haben, da $M \geq 1$ ist.
  \end{proof}
