...
\begin{equation*}
\begin{split}
& \phantom{\Leftrightarrow} x \leq \exp(x) + 1 \\
...
\end{split}
\end{equation*}
Wobei die letze Ungleichung aus der Eigenschaft des Betrags kommt, da $\frac{1}{1 + \exp(-x)} - 1< 0$ ist, weil $1 + \exp(-x) > 1$, da $\exp(-x) > 0$. Dies zeigt die erste Relation aus Bedingung (iii).
Die zweite Relation folgt durch die gleiche Art und Weise, da wir durch $$\frac{1}{1 + \exp(-x)} - \frac{1}{2}= \sigma(x + 0) - \frac{1}{2} = -\sigma(x - 0) + \frac{1}{2} = -\frac{1}{1 + \exp(-x)} + \frac{1}{2}$$
wissen, dass $\sigma$ punktsymmetrisch in $(0, \frac{1}{2})$ ist. Aus dieser Eigenschaft folgt mit $$\sigma(x) - 1 = \frac{1}{1 + \exp(-x)} - 1 = -\frac{1}{1 + \exp(-x)} = -\sigma(x)$$ für $x < 0$ aus der ersten Relation, da $|x| > 0 $ ist$\colon$  
$$|\sigma(x)| = |-\sigma(x)| = |\sigma(x) - 1| \leq \frac{1}{|x|}.$$
