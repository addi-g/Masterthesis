...
\end{defn}
Wir werden ab jetzt der Einfachheit halber nur noch von neuronalen Netzen mit Architektur $(L,\bk)$ reden und beziehen uns damit immer auf Definition~\ref{def:nn}.
\begin{bemnumber}
...
\end{itemize}
\end{bemnumber}
Da wir bei Neuronale-Netze-Regressionsschätzern Funktionswerte schätzen möchten, haben wir in der Ausgabeschicht nur ein Neuron. Damit erreichen wir einen eindimensionalen Output. Wie wir an der Konstruktion des neuronalen Netzes erkennen kann, nehmen wir in der Ausgabeschicht die Identität als Aktivierungsfunktion.
...
\begin{lem}[Lagrangesche Form des Restglieds, Kapitel 2.4 Seite 67 \cite{Königsberger2013}]
\label{lem:lagrange}
...
wobei das Restglied mit einem geeigneten Punkt $\xi \in [u;x]$ in der Form 
$$ R_{N + 1}(x;u) = \frac{f^{(N + 1)}}{(N + 1)!} (x - u)^{N + 1}$$
...
Wir betrachten erst eine Approximation der Identität durch das neuronale Netz $f_{\id}$ welches in Abbildung~\ref{fig:fid} veranschaulicht wird und anschließend die Approximation eines Quadrats auf einem Intervall durch das neuronale Netz $f_{\sq}$.
\begin{figure}[htp]
\centering
\begin{tikzpicture}[
plain/.style={
...
\begin{lem}
  \label{lem:1}
  Sei $\sigma \colon \R \to \R$ eine Funktion, $\|\cdot\|_{\infty}$ die Supremumsnorm auf $[-a, a]$ und $R$, $a > 0$.
  \begin{itemize}
...
\end{proof}
Wir betrachten als nächstes die Approximation einer Multiplikation zweier Werte auf einem Intervall durch das neuronale Netz $f_{\mult}$ welches in Abbildung~\ref{fig:fmult} veranschaulicht wird.
\begin{figure}[htp]
\centering
\begin{tikzpicture}[
plain/.style={
...
wobei wir bei der letzten Ungleichung Lemma~\ref{lem:1}b) mit $x + y, x - y \in [-2a, 2a]$ verwendet haben. Daher folgt insbesondere für beliebiges $x \in [-2a,2a]$ $$ |f_{\sq}(x) - x^2| \leq \frac{40 \cdot \|\sigma'''\|_{\infty} \cdot a^3}{3 \cdot |\sigma''(t_{\sigma})|} \cdot \frac{1}{R}$$ gilt. 
  \end{proof}
Wir betrachten als nächstes die Approximation der Maximumsfunktion $\max\{x, 0\}$ für $x$ aus einem Intervall, durch das neuronale Netz $f_{\ReLU}$  Abbildung~\ref{fig:frelu} veranschaulicht wird.
\begin{figure}[htp]
\centering
\begin{tikzpicture}[
plain/.style={
...
Daraus folgt die Behauptung.
  \end{proof}
Wir betrachten als nächstes die Approximation des Positivteils einer Funktion auf einer Intervall durch das neuronale Netz $f_{\mathrm{hat},y}$ welches Abbildung~\ref{fig:fhat} veranschaulicht wird.
\begin{figure}[htp]
\centering
\begin{tikzpicture}[
plain/.style={
