...
Mit $f_{\id}(0) = 0$ und $f_{\id}'(0) = 1$ erhalten wir:
  	...
  	\item[b)]  Die Funktion $f_{\sq}$ die hier nun 3-mal differenzierbar ist, da $\sigma$ nach Voraussetzung  3-mal stetig differenzierbar ist. Es wie in a) durch Lemma \ref{lem:lagrange} mit $f = f_{\sq}, N = 2, u = 0$ und $I = [-a, a]$, dass ein $\xi \in [0, x]$ existiert, so dass:
$$f_{\sq}(x) = \sum_{k = 0}^N \frac{f_{\sq}^{(k)}(u)}{k!}(x - u)^k + \frac{f_{\sq}^{(N + 1)}(\xi)}{(N + 1)!}(x - u)^{N + 1}.$$
